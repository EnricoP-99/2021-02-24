...
import it.polito.tdp.PremierLeague.model.EfficienzaP;
...
public class PremierLeagueDAO {
	...
	public List<EfficienzaP> getInfo(Match m, Map<Integer, Player> idMap)
	{
		...
		List<EfficienzaP> result = new ArrayList<EfficienzaP>();
		Connection conn = DBConnect.getConnection();
		...
		try {
			...
			while (res.next()) {
				...
				if(idMap.containsKey(res.getInt("p1")) && idMap.containsKey(res.getInt("p2"))) {
					result.add(new EfficienzaP(idMap.get(res.getInt("p1")), idMap.get(res.getInt("p2")), res.getDouble("peso")));
				}
